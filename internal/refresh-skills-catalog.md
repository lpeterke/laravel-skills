...
- **The lint toolchain** that `laravel-lint-setup` targets — three upstreams, all listed with what to compare in CLAUDE.md's *Upstream sources* section. Re-check each against `skills/laravel-lint-setup/SKILL.md`:
  - [laravel/livewire-starter-kit](https://github.com/laravel/livewire-starter-kit) — clone shallow and diff its `pint.json`, `phpstan.neon` (paths and `level`), and `composer.json`'s `scripts` block against Steps 2, 4 and 5 of the skill. This is the parity target; if the starter kit changes its level or renames a script, the skill is wrong until it follows.
